...
function initialize_resources($type, $resources)
{
...
function response_function($text, $code)
{
	echo json_encode(array("msj" => $text, "code" => $code));
}
...
function navigation_renderer($url, $nombre_pagina)
{
	echo '
            <div class="card mb-4">
            <div class="col-12">
                <div class="card-header">
                    <strong class="card-title"><a href="' . URL . '">Inicio</a> / <a href="' . URL . $url . '">' . $nombre_pagina . '</a></strong>
                </div>
            </div>
        </div>
            ';
}
function return_data_json($c)
{
	$data = array("data" => $c);
...
function title($text){
	echo '<h4 class="fw-bold py-3 mb-4">'.$text.'</h4>	';
}
function sub_title($title, $sub){
	echo '<h4 class="fw-bold py-3 mb-4"><span class="text-muted fw-light">'.$title.'/</span> '.$sub.'</h4>';
}
